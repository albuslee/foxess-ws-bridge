...
class FoxESSWebClient:
    ...
    def _headers(self, path: str) -> dict[str, str]:
        ...
        return _build_headers(path, self._get_token(), self._get_signature)
    ...
    async def _post(self, path: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if data.get("errno") != 0:
            _LOGGER.error(
                ...
                path, data.get("errno"), data.get("msg"),
            )
            raise WebApiError(
                f"Web API {path}: errno={data.get('errno')}, msg={data.get('msg')}"
            )
    ...
    async def update_peak_fdpwr(self, fdpwr: int) -> int:
        """Read current scheduler, update fdpwr on the 18:00-19:59 group, write back.
        ...
        updated = 0
        for g in groups:
            if g.get("startHour") == 18 and g.get("endHour") == 19:
                g["fdpwr"] = fdpwr
                updated += 1

        if updated == 0:
            raise WebApiError("No 18:00-19:59 group found in current scheduler")

        # Write back the full list (already in web format)
        payload = {
...
async def resolve_device_id(
    ...
    headers = _build_headers(WEBAPI_PLANT_DEVICES, token, get_signature)
    ...
    _LOGGER.debug(
        ...
        query_path, WEBAPI_PLANT_DEVICES,
    )
    ...
    try:
        ...
        for device in devices:
            if device.get("sn") == device_sn:
                ...
                _LOGGER.info(
                    "Resolved deviceID for %s: %s", device_sn, device_id,
                )
                return device_id
